...
- **Abrir el editor por `adb` en el teléfono:** el intent `SEND` desde el shell no concede permiso sobre el URI; sin `pm grant com.tharunbirla.librecuts android.permission.READ_MEDIA_VIDEO` la app truena con LC-500 (M-255). El video sintético se genera con `ffmpeg -f lavfi -i testsrc2...` en la PC, se sube a `/sdcard/Movies/` y se indexa con `am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE`.
- **El editor oculta las barras del sistema por defecto** (modo inmersivo, preferencia `fullscreen_editor`). Al revisar insets, prueba también con la preferencia desactivada: se cambia editando `shared_prefs/librecuts_prefs.xml` con `run-as` (sube el archivo a `/data/local/tmp` y cópialo con `run-as ... sh -c 'cat ... > shared_prefs/...'`; nunca redirijas sobre el archivo de prefs algo que pueda fallar, porque lo vacía).
- **Activities no exportadas** (`ErrorDisplayActivity`) no se abren con `am start` ni con `run-as`: para capturarlas, un test instrumentado temporal que las lance y espere.
